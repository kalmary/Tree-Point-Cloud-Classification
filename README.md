...
│   ├── EvalSegm_ResNet.py    #Main Python Program for evaluating outputs
...
To evaluate trained model, run EvalSegm_ResNet.py with proper flags:

```bash
cd src/model_pipeline
python src/model_pipeline/EvalSegm_ResNet.py --model_name MODEL_NAME --device cuda --mode 1
```
